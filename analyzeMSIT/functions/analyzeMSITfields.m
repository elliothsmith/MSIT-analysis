function [fieldStats,rejectTrials] = analyzeMSITfields(patientID,sessionNum,nevFile,tf)
...
for aS = 1
    ...
    for ch = 1:length(LFPlabels)
        ...
        for tt = 1:length(goods)
            ...
            %% for Brian: the LFP mat above is the epoched LFP data. 
            ...
            Sf_encodingWindow(:,tt) = nanmean(Stf(tspec>0 & tspec<RTs(tt),:,tt));
...
%                 keyboard
                ...
                SfdB = nanmean(Sf_encodingWindow(:,trialType==1)./repmat(Sf_baseline,1,sum(trialType==1)),2);
                ...
                Sferr = nanstd(Sf_encodingWindow(:,trialType==1)./repmat(Sf_baseline,1,sum(trialType==1)),[],2)./sqrt(sum(trialType==1));
...
%                 display(sprintf('figure saved as %s\n\n        ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~        ',fName));
                ...
                plot(f,(Sf2dB-Sf2err)','color',col2,'linestyle','--')
                % plotting means
                plot(f,SfdB,'color',col0,'linewidth',2)
...
%                patch([f fliplr(f)],[SfdB'+Sferr' fliplr(SfdB'-Sferr')],[0.2 0.2 0.2],'edgecolor','none','facealpha',0.3)
        ...
        print(handleF,sprintf('~/data/msit_units/Experiment_I__ACC/%s/Figs/spectrograms/%s',patientID,fName),'-dpdf')
        close(handleF)
