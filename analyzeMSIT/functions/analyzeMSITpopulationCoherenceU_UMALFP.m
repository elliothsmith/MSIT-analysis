function [coherenceStats] = analyzeMSITpopulationCoherenceU_UMALFP(patientID,sessionNum,nevFile)
...
rejectTrials = dischargeRejectionToolU_UMALFP(patientID,sessionNum,nevFile,0);
...
movingWin = [1 0.025];
params.Fs = 500; % sampling frequency for LFP
params.pad = 0; %
...
for aS = 1
    ...
    for tt = 1:nTrials
        if floor((trialStarts(tt)+post)*params.Fs)<length(dsData)
            %% tensorizing LFP:
...
condition = condition(1:length(trialType));
...
tmp = trialType.*notRejected;
trialTypeRJCTD = tmp(logical(notRejected));
...
for aS2 = 1:length(data)
    ...
    savedFileName = sprintf('/media/user1/data4TB/data/msit_units/Experiment_II__dlPFC/%s/Data/populationCoherence/%sAlignedCoherence_averagedCohgramsOverUnits_session%d_continuousChannel%s.mat',patientID,alignName,sessionNum,LFPlabels);
