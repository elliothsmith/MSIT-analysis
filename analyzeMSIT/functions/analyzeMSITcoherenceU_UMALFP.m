function [coherenceStats] = analyzeMSITcoherenceU_UMALFP(patientID,sessionNum,nevFile)
...
if strcmp(patientID,'CUCX2')
    LFPlabels = 'meanUMA'; % anterior ECoG contact to the array.
end
...
for aS = aSvec
    ...
    for tt = 1:nTrials
        ...
        %% tensorizing LFP:
        if floor((trialStarts(tt)+post)*params.Fs)<size(dsData,2)
            LFPmat{aS}(:,tt) = dsData(floor((trialStarts(tt)-pre)*params.Fs):floor((trialStarts(tt)+post)*params.Fs));
        end
...
%             tmpType = trialType.*notRejected;
            ...
            if isequal(params.trialave,0)
                ...
                savedFileName = sprintf('/media/user1/data4TB/data/msit_units/Experiment_II__dlPFC/%s/Data/coherence/%sAlignedCoherence_singleTrialCohgrams_session%d_unit%d_channel%d_continuousChannel%s.mat',patientID,alignName,sessionNum,un,ch,LFPlabels);
...
%                 [nullDist,nullPhase] = generateCoherenceNullDist(500, squeeze(LFPmat{aS2}(:,:)), data(aS2).channel(ch).unit(un).trial, movingWin, params);
                ...
                minval = nanmin(nanmean(nanmean(C_alltrials)));
                maxval = nanmax(nanmean(nanmean(C_alltrials)));
            ...
            elseif isequal(params.trialave,1)
                ...
                savedFileName = sprintf('/media/user1/data4TB/data/msit_units/Experiment_II__dlPFC/CUCX2/Data/coherence/%sAlignedCoherence_averagedCohgrams_session%d_unit%d_channel%d_continuousChannel%s.mat',patientID,alignName,sessionNum,un,ch,LFPlabels);
