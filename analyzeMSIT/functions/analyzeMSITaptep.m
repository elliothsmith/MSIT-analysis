function [aptepAnova] = analyzeMSITaptep(patientID,sessionNum,nevFile)
%ANALYZEMSITAPTEP creates action potential-triggered evoked potentials.
%
%   [aptepStats] = analyzeMSITaptep(patientID,sessionNum,nevFile)
...
for aS = 1
    ...
    for ch = 1:nChans
        ...
        for un = 1:nUnits
            ...
            tmp = deblank(data(aS).channel(ch).label);
            ...
            %             %% [20161021] initializing the APmat (:,ts,tt,cCH)
            %             tmpAPmat{1} = zeros(2*spikeWin*params.Fs+1,nTrials,length(contChans))
            
            
            % for medial and lateral LFP channels.
            for cCH = contChans
                ...
                %% labels from current session
                tmpt = deblank(LFPlabels{contChans(1)});
                currentTrode = tmpt(1:end-1);
                ...
                for tt = 1:nTrials
                    ...
                    % the spikes taht are just after the cue.
                ...
                for tt2 = 1:nTrials
                    if isempty(relevantEvents{tt2})
                        tmpAPmat(:,tt2,cCH) = nan(2*spikeWin*params.Fs+1,1);
                    else
                        for zz = 1:length(relevantEvents{tt2})
                            tmptmpAPmat(:,zz) = dData(cCH,round(relevantEvents{tt2}(zz)-spikeWin)*params.Fs:round(relevantEvents{tt2}(zz)+spikeWin)*params.Fs);
                        end
                        tmpAPmat(:,tt2,cCH) = mean(tmptmpAPmat(:,zz),2);
                        clear tmptmpAPmat
                    end
                end
...
%                 keyboard
                ...
                aptep.lfpChannelLocation{ch,un} = LFPcohLoc;
                aptep.lfpChannelLlabel{ch,un} = tmpt;
                ...
                hold on
                plot(tsec,squeeze(nanmean(tmpAPmat(:,:,cCH),2)),'color','k','linewidth',1,'linestyle',lS)
                plot(tsec,squeeze(nanmean(tmpAPmat(:,trialType==2,cCH),2)),'color',col1a,'linewidth',1,'linestyle',lS)
                plot(tsec,squeeze(nanmean(tmpAPmat(:,trialType==3,cCH),2)),'color',col1b,'linewidth',1,'linestyle',lS)
                plot(tsec,squeeze(nanmean(tmpAPmat(:,trialType==1,cCH),2)),'color',col0,'linewidth',1,'linestyle',lS)
                plot(tsec,squeeze(nanmean(tmpAPmat(:,trialType==4,cCH),2)),'color',col2,'linewidth',1,'linestyle',lS)
                ...
                for stp = 1:numSteps
                    ...
                    [p,aptepAnova(stp).anovatab,aptepAnova(stp).stats] = anova1(aptepAll(stpIdx,:,cCH),trialType','off');
                ...
                [S0,t,f] = mtspecgramc(tmpAPmat(:,trialType==1,cCH),movingWin,params);
                [S1a,t,f] = mtspecgramc(tmpAPmat(:,trialType==2,cCH),movingWin,params);
                [S1b,t,f] = mtspecgramc(tmpAPmat(:,trialType==3,cCH),movingWin,params);
                [S2,t,f] = mtspecgramc(tmpAPmat(:,trialType==4,cCH),movingWin,params);
                tspec = linspace(-spikeWin,spikeWin,length(t));
                %                 keyboard
                
                
                %% plotting spectral decomposition of APtEPs.
                plotmultipleaxes(plotIdcs(1),4,3,0.05,ch*1000+un)
                hold on
%                 imagesc(tspec,f,(S0./repmat(f.^-2,size(S0,1),1))')
%                 line([0 0],params.fpass,'color','k','linestyle','--','linewidth',2)
%                 text(-1,params.fpass(2)-10,sprintf('none%s',LFPcohLoc))
%                 hold off
%                 axis tight xy square
%                 set(gca,'linewidth',1.5,'fontsize',14)
%                 plotmultipleaxes(plotIdcs(2),4,3,0.05,ch*1000+un)
%                 hold on
%                 imagesc(tspec,f,(S1a./repmat(f.^-2,size(S1a,1),1))')
%                 line([0 0],params.fpass,'color','k','linestyle','--','linewidth',2)
%                 text(-1,params.fpass(2)-10,sprintf('simon%s',LFPcohLoc))
%                 hold off
%                 axis tight xy square
%                 set(gca,'linewidth',1.5,'fontsize',14)
%                 plotmultipleaxes(plotIdcs(3),4,3,0.05,ch*1000+un)
%                 hold on
%                 imagesc(tspec,f,(S1b./repmat(f.^-2,size(S1b,1),1))')
%                 line([0 0],params.fpass,'color','k','linestyle','--','linewidth',2)
%                 text(-1,params.fpass(2)-10,sprintf('eriksen%s',LFPcohLoc))
%                 hold off
%                 axis tight xy square
%                 set(gca,'linewidth',1.5,'fontsize',14)
%                 plotmultipleaxes(plotIdcs(4),4,3,0.05,ch*1000+un)
%                 hold on
%                 imagesc(tspec,f,(S2./repmat(f.^-2,size(S2,1),1))')
%                 line([0 0],params.fpass,'color','k','linestyle','--','linewidth',2)
%                 text(-1,params.fpass(2)-10,sprintf('both%s',LFPcohLoc))
%                 hold off
%                 axis tight xy square
%                 set(gca,'linewidth',1.5,'fontsize',14)
%                 
%                 colormap jet
            ...
            end % for medial and lateral LFP channels.
%             handleF = ch*1000+un;
%             
%             %         %% saving figures.
%             %         try
...
%             if exist('./Figs','dir')
%                 try
%                     fName = sprintf('./Figs/APtEP/%s_Data03_session_%d_channel%d_unit%d_LFPChannel_%s_APtEP_denoisedWith%s',patientID,sessionNum,ch,un,deblank(LFPlabels{cCH}),denoiseMethod);
%                     %                     saveas(handleF,fName,'pdf')
...
%                 close(handleF)
%             else
%                 fName = sprintf('%s_Data03_session_%d_channel%d_unit%d_LFPChannel_%s_APtEP_denoisedWith%s',patientID,sessionNum,ch,un,deblank(LFPlabels{cCH}),denoiseMethod);
%                 %                     saveas(handleF,fName,'pdf')
%                 print(fName,'-dpdf','-fillpage')
%                 close(handleF)
%             end
